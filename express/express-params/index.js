...
app.get("/users", (req, res) => {
    ...
    if (pincode) {
      const filterByPincodes = arr.filter(user => user.pincode === pincode);
        const filterByStates = filterByPincodes.filter(user => user.state === state);
        
        
      if (filteredUsers.length === 0) {
        return res.status(404).json({ message: "No users found with provided pincode" });
      }
      return res.status(200).json({ message: "Users found", data: filteredUsers });
    }
